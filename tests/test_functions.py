...
class TestChallenge(unittest.TestCase):
    ...
    def test_find_minimum(self):
        """
        Write a function `find_minimum(numbers)` that returns the smallest number in a list.
        Handle the case for empty lists (return None).
        """
        self.assertEqual(find_minimum([4, 2, 9, 1]), 1)
        self.assertEqual(find_minimum([-5, -1, -10, 0]), -10)
        self.assertIsNone(find_minimum([]))

    def test_calculate_factorial(self):
        """
        Write a function `calculate_factorial(n)` that calculates the factorial of a non-negative integer.
        Use an iterative approach (no recursion).
        """
        self.assertEqual(calculate_factorial(5), 120)
        self.assertEqual(calculate_factorial(0), 1)  # Edge case: factorial of 0 is 1
        self.assertEqual(calculate_factorial(7), factorial(7))  # Match Python's math.factorial

    def test_unique_elements(self):
        """
        Write a function `unique_elements(items)` that returns a list of unique elements, maintaining their order.
        """
    ...
    def test_check_palindrome(self):
        """
        Write a function `check_palindrome(string)` that returns True if the input string is a palindrome, False otherwise.
        Ignore case and spaces.
        """
        self.assertTrue(check_palindrome("racecar"))
        self.assertTrue(check_palindrome("A man a plan a canal Panama"))#edge case
        self.assertFalse(check_palindrome("hello"))

    def test_fibonacci_sequence(self):
        """
        Write a function `fibonacci_sequence(n)` that returns the first `n` numbers in the Fibonacci sequence.
        Start with 0 and 1.
        """
